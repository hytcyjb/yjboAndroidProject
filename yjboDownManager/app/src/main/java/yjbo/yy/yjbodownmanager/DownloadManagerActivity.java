...
import yjbo.yy.yjbodownmanager.downmanager.task.ExecutorWithListener;
        ...
        private void refresh() {
            ...
            if (downloadInfo.getState() == DownloadManager.NONE) {
                ...
                if (ApkUtils.isAvailable(DownloadManagerActivity.this, new File(downloadInfo.getTargetPath()))) {
                    ...
                    if (downloadInfo.getTargetPath().indexOf(".mp4") != -1) {
                        download.setText("播放");
                    } else if (downloadInfo.getTargetPath().indexOf(".apk") != -1) {
                        download.setText("安装");
                    } else {
                        download.setText("未知");
                    }
                ...
                switch (downloadInfo.getState()) {
                    ...
                    case DownloadManager.FINISH:
                        if (ApkUtils.isAvailable(DownloadManagerActivity.this, new File(downloadInfo.getTargetPath()))) {
                            ...
                            if (downloadInfo.getTargetPath().indexOf(".mp4") != -1) {
                                //参考：http://blog.csdn.net/jw20082009jw/article/details/54583115
                                Uri uri = Uri.parse("file://" + downloadInfo.getTargetPath());
                                //调用系统自带的播放器
                                ...
                                Toast.makeText(DownloadManagerActivity.this, "未知类型的内容,无法解析", Toast.LENGTH_SHORT).show();
                            }
